...
int getChild(void)
{
    ...
    list_for_each(list, &task->children)
    {
        ...
        printk(KERN_INFO "Process ID of child : %d, State of process : %s\n", child_process->pid, getStatus(child_process->__state));
    }
    return 0;
}
...
{
    printk(KERN_INFO "Module lkm2 inserted successfully\n");
    getChild();
    return 0;
}
...
{
    printk(KERN_INFO "Module lkm2 exited successfully\n");
    return;
}
