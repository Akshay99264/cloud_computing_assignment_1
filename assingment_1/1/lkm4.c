...
MODULE_DESCRIPTION("Kernel Module print total virtual and physical memory allocated to given process");
...
static int func_init(void)
{
    ...
    unsigned long physical_address;
    unsigned long total_virtual_memory_size = 0, total_physical_memory_size = 0;
    printk(KERN_INFO "Module lkm4 inserted successfully\n");
    ...
    {
        total_virtual_memory_size += vma->vm_end - vma->vm_start;
        for (unsigned long it = vma->vm_start; it < vma->vm_end; it++)
        {
            physical_address = get_physical_address(mm, it);
            if (physical_address)
            {
    ...
    printk(KERN_INFO "Virtual memory: %lu, Physical memory: %lu\n", total_virtual_memory_size, total_physical_memory_size);
...
{
    printk(KERN_INFO "Module lkm4 exited successfully\n");
    return;
}
