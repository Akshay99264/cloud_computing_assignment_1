...
{
    printk(KERN_INFO "ioctl driver get opend\n");
    return 0;
}
static int my_close(struct inode *i, struct file *f)
{
    printk(KERN_INFO "ioctl driver get closed\n");
    return 0;
}
...
static long my_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    case QUERY_GET_PHYSICAL_ADDR:
    {
        ...
        PID = temp.pid;
        virtual_address = temp.virtual_address;
    ...
    case QUERY_CHANGE_VALUE:
    {
        ...
        *(char *)phys_to_virt(temp.physical_address) = temp.value; // modify the value given physical address.
        printk(KERN_INFO "Modification Successfull\n");
        break;
    }
    default:
        return -EINVAL;
...
static int __init query_ioctl_init(void)
{
    ...
    printk(KERN_INFO "Module solution_2_I inserted successfully\n");
...
static void __exit query_ioctl_exit(void)
{
    ...
    cdev_del(&c_dev);
    unregister_chrdev_region(dev, MINOR_CNT);
    printk(KERN_INFO "Module solution_2_I exited successfully\n");
}
